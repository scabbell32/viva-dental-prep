...
async function main() {
  console.log('Retrieving untranslated active questions...')

  const { data: allQuestions, error: qErr } = await supabase
    .from('questions')
    .select('id, question_text, option_a, option_b, option_c, option_d, option_e, option_f, explanation, question_text_es, is_active, week_number')
    .eq('is_active', true)

  if (qErr) throw qErr
  const untranslatedQs = (allQuestions ?? []).filter(q => !q.question_text_es)

  console.log(`Found ${untranslatedQs.length} untranslated active questions.`)

  let count = 0
  for (const q of untranslatedQs) {
    try {
      count++
      await translateQuestion(q)
      console.log(`Progress: ${count}/${untranslatedQs.length}`)
      // Brief delay to avoid rate limit spikes
  ...
  console.log('Auto-translation run finished!')
}
